...
namespace CryptoTrade.Services
{
    public class TransactionLogService : ITransactionLogService
    {
        ...
        {
            var Tran_log = await _context.TransactionLogs
                .OrderBy(x=>x.Date)
                .FirstOrDefaultAsync(x => x.Id.ToString() == tranid) ?? throw new Exception($"Transaction with {tranid} not found"); ;
            var return_val =_mapper.Map<TransactionLogGetDetailedDto>(Tran_log);
            return_val.CryptoName = 
                await _context.Cryptos
                .Where(x => x.Id.ToString() == Tran_log.CryptoId)
                .Select(x => x.Name)
                .FirstOrDefaultAsync() ?? throw new Exception($"Crypto with {Tran_log.CryptoId} not found");
            return return_val;
        }
